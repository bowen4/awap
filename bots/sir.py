...
class BotPlayer(Player):
    ...
    def __init__(self, map: Map):
        ...
        self.gs_poss = set()
        self.sf_poss = set()
        self.tower_coords = set()
        self.enemy_strength = deque()
        for (x, y) in map.path:
        ...
        while len(self.gs_poss) >= 3 * len(self.sf_poss):
            self.sf_poss.append(self.gs_poss.pop(0))
        self.sf_poss.sort(key=lambda x : x[0] + x[1])
    ...
    def play_turn(self, rc: RobotController):
        if self.isRush:
            ...
            return
        us = rc.get_ally_team()
        if rc.get_time_remaining_at_start_of_turn(us) <= 2.0:
        ...
        while True:
            if self.next_action == 0:
                ...
                for (dx, dy) in product(range(-2, 3), range(-2, 3)):
                    if dx**2 + dy**2 > 5:
                        continue
                    if (x + dx, y + dy) in self.tower_coords:
                        count += 1
                if count >= 9:
                    self.next_action = 10
                    self.rein_coords = (x, y)
                    self.gs_poss.pop()
                    continue
                if rc.can_build_tower(TowerType.GUNSHIP, x, y):
                    ...
                    break
            elif 3 <= self.next_action < 7:
                if rc.get_health(us) < 2500 and rc.get_turn() < 700:
                    self.next_action = 0
                    continue
                if not self.sf_poss:
    ...
    def compute_enemy_strength(self, rc: RobotController):
        them = rc.get_enemy_team()
        towers = rc.get_towers(them)
        strength = 0
        for tower in towers:
            if tower.type == TowerType.BOMBER:
                strength += TowerType.BOMBER.cost
            if tower.type == TowerType.GUNSHIP:
                strength += TowerType.GUNSHIP.cost
            if tower.type == TowerType.SOLAR_FARM:
                strength += TowerType.SOLAR_FARM.cost
            if tower.type == TowerType.REINFORCER:
                strength += TowerType.REINFORCER.cost
            self.enemy_strength.append(strength)
        if len(self.enemy_strength) > 5:
            old_strength = self.enemy_strength.popleft()
            if old_strength > 1.3 * strength:
                us = rc.get_ally_team()
                our_towers = rc.get_towers(us)
